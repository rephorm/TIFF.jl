...
-------------
  ...
  Four image types: bilevel, grayscale, palette-color, full-color

  Required Bilevel Tags:

    + Photometric Interpretation (how black/white are encoded)
      262 (106.H) SHORT
        0 = WhiteIsZero
        1 = BlackIsZero
    ...
    + Compression
      259 (103.H) SHORT
        1 = No compression (packed bits)
        2 = Huffman
        32773 = PackBits compression

    + ImageLength (number of scanlines)
      257 (101.H) SHORT or LONG

    + ImageWidth (number of columns -- pixels per scanline)
      256 (100.H) SHORT or LONG
    ...
    + XResolution (number of pixels per ResolutionUnit in ImageWidth)
      282 (11A.H) RATIONAL

    + YResolution (number of pixels per ResolutionUnit in ImageLength)
      283 (11B.H) RATIONAL

    + RowsPerStrip (rows in each strip exact possibly last)
      278 (116.H) SHORT or LONG

    + StripOffsets (byte offset of each strip)
      273 (111.H) SHORT or LONG

    + StripByteCounts (number of bytes in each strip *after compression*)
      279 (117.H) SHORT or LONG

  Changes for Greyscale:

    * Compression (value 2 is not allowed)

    + BitsPerSample (4 or 8 in spec)
      258 (102.H) SHORT


  Changes for Palette-color Images

    * PhotometricInterpretation = 3 (Palette Color)

    + Colormap (RGB lookup table)
      320 (140.H) SHORT
        N = 3 * (2**BitsPerSample) [R,G,B for each possible sample value]
        0,0,0 = black
        65535,65535,65535 = white

  Changes for RGB Images
    ...
    * PhotometricInterpretation = 2
    ...
    + SamplesPerPixel
      277 (115.H) SHORT
        (if > 3, then see ExtraSamples field)
